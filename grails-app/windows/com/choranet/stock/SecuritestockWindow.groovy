...
class SecuritestockWindow extends SuperWindow {
    ...
    public SecuritestockWindow (parametrageService) {
        super(Securitestock.class, 11, true, true)
    ...
    def entrepotChoisi() {
        ...
        if (!produitPerissableManaged && entrepotSelected != null){
            produits = securitestockService.getProduitsSansSecuriteStock(entrepotSelected)
        }else {
            produits = securitestockService.getProduitsEnStock(entrepotSelected)   
        }
        if(produits != null) {
            produits = produits.sort{it.designation}
            if (produits.size() == 0) {
    ...
    def initialiserAssociation() {
        ...
        listeProduits = Produit.list().sort{it.designation}
    }
    /**
    ...
    def afficherValeurAssociation() {
        ...
        def binderentrepot = new AnnotateDataBinder(this.getFellow("coentrepots"))
        entrepots = Entrepot.list()
        entrepotSelected = objet.entrepot
        binderentrepot.loadAll()
                       
        def binderproduit = new AnnotateDataBinder(this.getFellow("coproduits"))
        produits = securitestockService.getProduitsEnStock(entrepotSelected).sort{it.designation}  
        produitSelected = produits.find{it.id == objet.produit.id}
        binderproduit.loadAll()
    ...
    def addOrUpdate(){
        def isProduitFoundInStock = false
        //if (produitPerissableManaged) {
            isProduitFoundInStock = securitestockService.isProduiEnEntrepotPourDatePreremptionFound(produitSelected, entrepotSelected, objet.date_preremption)
        //}
        if (isUpdate || isProduitFoundInStock) {
            def oldObjet = securitestockService.getSecuriteStockEnEntrepotPourDLC(produitSelected, entrepotSelected, objet.date_preremption)
            oldObjet.stockMin = objet.stockMin
            oldObjet.stockMax = objet.stockMax
            oldObjet.stockEntre = objet.stockEntre
            oldObjet.stockSortie = objet.stockSortie
            objet = oldObjet
            this.update()
            isUpdate = false
        } else {
    ...
    def changementProduit(prodSelected) {
        def codeProd = prodSelected.split(":")[1]
        codeProd = codeProd.substring(1)
        produitSelected = Produit.findByCodeLike(codeProd)
        objet = securitestockService.getQuantiteProduitEnEntrepot(produitSelected, entrepotSelected, objet)
        new AnnotateDataBinder(this.getFellow("fieldStockReel")).loadAll()
        new AnnotateDataBinder(this.getFellow("fieldStockEntre")).loadAll()
        new AnnotateDataBinder(this.getFellow("fieldStockSortie")).loadAll()
    }
    
    def changementDlcProduit(dlcSelected) {
        objet = securitestockService.getQuantiteProduiEnEntrepotPourDatePreremption(produitSelected, entrepotSelected, dlcSelected, objet)
        new AnnotateDataBinder(this.getFellow("fieldStockReel")).loadAll()
        new AnnotateDataBinder(this.getFellow("fieldStockEntre")).loadAll()
        new AnnotateDataBinder(this.getFellow("fieldStockSortie")).loadAll()
    }
    ...
    def updateFieldDLC(){
        if(produitPerissableManaged){
            def fieldDatePreRemptionIsDisabled = true
            if(produitSelected.perissable != null) {
                fieldDatePreRemptionIsDisabled = !produitSelected.perissable
            }
            this.getFellow("fieldPreRemption").disabled = fieldDatePreRemptionIsDisabled
            if (fieldDatePreRemptionIsDisabled) {
                this.getFellow("fieldPreRemption").constraint = null
                this.getFellow("fieldPreRemption").value = null
            }
        }
    }
}
