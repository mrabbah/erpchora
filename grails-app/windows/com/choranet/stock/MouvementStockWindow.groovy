...
class MouvementStockWindow extends SuperWindow {
    ...
    def actualiserValeurAssociation() {
        ...
        produits = securitestockService.getProduitsEnStock(objet.empSource).sort{it.designation}
        ...
        if(empDestinations == null || empDestinations.size() == 0) {
            empDestinationSelected = null
            //empDestinationSelected = empDestinations.get(0)
        }
        ...
        //        objet.prixDeduitMvm = prixMvmSelected
    ...
    def getDLCs(){
        if (produitPerissableManaged){
            def dlcsFlow = this.getFellow("codlcs")
            def fieldDatePreRemptionIsDisabled = true
            def binderDlcs = new AnnotateDataBinder(dlcsFlow)
            dlcs = null
            ...
            if(produitSelected != null){
                ...
                if ((dlcs != null) && (dlcs.size() == 1)){
                    dlcSelected = dlcs[0]
                }
            }
            dlcsFlow.disabled = fieldDatePreRemptionIsDisabled
            if (fieldDatePreRemptionIsDisabled) {
                dlcsFlow.constraint = null
                dlcsFlow.value = null
            }
            binderDlcs.loadAll()
        }
...
//    }
    ...
    def updateProduitListAndEmpDestination(empSourceSelected){
        ...
        produits = securitestockService.getProduitsEnStock(empSourceSelected).sort{it.designation}
        def binderproduit = new AnnotateDataBinder(this.getFellow("coproduits"))
        binderproduit.loadAll()
        
        //        this.getFellow("codlcs").value = null
        //        new AnnotateDataBinder(this.getFellow("codlcs")).loadAll()
        //        
        //        this.getFellow("fieldquantitebis").value = null
        //        new AnnotateDataBinder(this.getFellow("fieldquantitebis")).loadAll()
        
        empDestinations = Entrepot.remove(Entrepot.list(), empSourceSelected)
        def binderempDestination = new AnnotateDataBinder(this.getFellow("coempDestinations"))
        binderempDestination.loadAll()
    }
...
//    }
    ...
    def add() {
        ...
        try {     
            getService().ajouterMouvementTransfert(objet)
        } catch(Exception ex) {
    ...
    def updateFieldDLC(){
        if(produitPerissableManaged){
            ...
            this.getFellow("codlcs").disabled = fieldDatePreRemptionIsDisabled
            if (fieldDatePreRemptionIsDisabled) {
                this.getFellow("codlcs").constraint = null
                this.getFellow("codlcs").value = null
            }
        }
    }
