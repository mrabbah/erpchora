...
class DevisServiceTests extends GrailsUnitTestCase {
    ...
    void testCompare() {
        ...
        def testService=new DevisService()
        def res=testService.compare(testInstanceslp[0],testInstanceslp[1])
        System.out.println("vgvghvg   "+res)
    }
    ...
    void testSave()  {
        ...
        testInstanceslp[0].save(flush:true)
          ...
          def testInstances1p = [new PaternCompteur(libelle : "Pattern bon commande", prefixe : "BC{annee}{mois}{jour}", suffixe : "",pas:1,remplissage:12,numeroSuivant:2290,type:"DEVIS")]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(PaternCompteur, testInstances1p)
...
paternCompteurControl.demand.getProchainNumDevisEtIncrementer(1..1) {it -> return  testId}
testService.paternCompteurService = paternCompteurControl.createMock()
       testService.save(testInstances[0])
    }
    void  testTrierParSequenceLigne() {
         def ex=new Integer(30032013)
        final SimpleDateFormat sdf_yyyyMMdd = new SimpleDateFormat("yyyy-MM-dd");
		final SimpleDateFormat sdf_ddMMyyyy = new SimpleDateFormat("dd/MM/yyyy");
       ...
       def g=testService.trierParSequenceLigne(testInstances[0])
        System.out.println("le resultat "+g.getClass()+"   "+g)
        assertTrue g[0] instanceof LigneProduit
    ...
    void testGetSequenceSuivante() {
        ...
        def testId="dh"
        def testService=new DevisService()
        def resultat=testService.getSequenceSuivante(testInstances[0])
         System.out.println("le resultat "+resultat.getClass()+"   "+resultat)
//        assertTrue g[0] instanceof LigneProduit
        assertEquals 3,resultat
        
    }
    
     void testGetDevisNonLiesBcLiesPartenaire() {
...
Devis.metaClass.static.createCriteria = { myCriteria }
       def ge=testService.getDevisNonLiesBcEtLies(testInstances[0])
       System.out.println("hghjhjfgjffff devis non lie"+ge.getClass()+"hchc "+ge)  
       assertEquals 1,ge.size()
       assertTrue ge[0] instanceof Devis
       assertEquals ge[0].numDevis,"dh"
