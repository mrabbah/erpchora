...
class ReglePrixTests extends GrailsUnitTestCase {
    ...
    void testReglePrixContraintes() {
       ...
       def testInstances9 = [new Partenaire(code : "005", raisonSociale : "celibataire", idFiscal : null,patente:"15165165",registreCommerce:"commerce",cnss:"fk13252152",adresse:"oulfa",codePostal:"20200",telephone:"0673327392",fax:"0632972654",email:"contact@choranet.com",siteWeb:"www.choranet.com",adresseFacturation:"hay el hana",adresseLivraison:"ain sebaii",estClient:new Boolean(true),estFournisseur:new Boolean(false),estParticulier:new Boolean(false),estBlacklist:new Boolean(false),plafond:new Double(12.12),modeRelancePaiement:testInstances2[0],modeReglementDefaut:testInstances3[0],echeance:testInstances4[0],ville:testInstances6[0],formeJuridique:testInstances7[0],modeLivraison:testInstances8[0],categoriePartenaires:testInstances1,solde:new Double(15.01),compteVerrouile:new Boolean(true),zone:testInstancesz[0],commercial:testInstancesee[0])]        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        ...
        assertEquals 1, ReglePrix.count()
        assertTrue testInstances[0] instanceof ReglePrix        
        def ReglePrix2 = new ReglePrix()
        assertFalse ReglePrix2.validate()
        assertEquals "nullable", ReglePrix2.errors["code"]
        assertEquals "nullable", ReglePrix2.errors["tauxRemise"]
        assertEquals "nullable", ReglePrix2.errors["libelle"]
        assertEquals "nullable", ReglePrix2.errors["priorite"]
        assertEquals "nullable", ReglePrix2.errors["typePrixdeBase"]
        assertEquals "nullable", ReglePrix2.errors["typeRegle"]
        assertEquals "nullable", ReglePrix2.errors["active"]
        assertEquals "nullable", ReglePrix2.errors["arrondirApresVirguel"]
        def ReglePrix3 = new ReglePrix(code : "004", tauxRemise : new Double(14.12), libelle : "5155",operateurComparaison:"ajout",quantiteMin:new Double(15.12),quantiteMax:new Double(15.12),priorite:new Integer(5),valeurFixe:new Double(15.12),dateDebut:sqlDate1,dateFin:sqlDate1,active:new Boolean(true),typePrixdeBase:"normale",typeRegle:"normale",arrondirApresVirguel:new Boolean(true),senseArrondissement:"grandeur",seuilArrondissement:new Double(15.12),categoriePartenaires:testInstances1,partenaires:testInstances9,categorieProduits:testInstancescp,produits:testInstances20)
        assertFalse ReglePrix3.validate()
        assertEquals "unique", ReglePrix3.errors["code"]
        assertEquals "unique", ReglePrix3.errors["libelle"]
        def ReglePrix4 = new ReglePrix(code : "005", tauxRemise : new Double(14.12), libelle : "51555",operateurComparaison:"ajout",quantiteMin:new Double(15.12),quantiteMax:new Double(15.12),priorite:new Integer(5),valeurFixe:new Double(15.12),dateDebut:sqlDate1,dateFin:sqlDate1,active:new Boolean(true),typePrixdeBase:"normale",typeRegle:"normale",arrondirApresVirguel:new Boolean(true),senseArrondissement:"grandeur",seuilArrondissement:new Double(1.0),categoriePartenaires:testInstances1,partenaires:testInstances9,categorieProduits:testInstancescp,produits:testInstances20)
        assertTrue ReglePrix4.validate()
        ReglePrix4.save(flush:true)
        assertEquals 2, ReglePrix.count()
        assertTrue ReglePrix.get(ReglePrix4.id) instanceof ReglePrix
        assertEquals "005" + " : " + "normale" + "(" + "5" + ")", ReglePrix4.toString()
        assertTrue ReglePrix4.quantiteMin >= 0d
        assertTrue ReglePrix4.quantiteMax >= 1d
        assertTrue ReglePrix4.priorite >= 1
        assertTrue (ReglePrix4.seuilArrondissement  >= 0.0 ||ReglePrix4.seuilArrondissement <= 1.0)
    }
}
