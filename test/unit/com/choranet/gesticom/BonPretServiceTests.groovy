...
class BonPretServiceTests extends GrailsUnitTestCase {
    ...
    void testSave() {
     ...
     def testInstances1c = [new CategoriePartenaire(libelle : "propr", categorieParente : null)]
        ...
        def testInstancesbp = [new BonPret(numBP : "bp001", type : "ACHAT", date : sqlDate1,reference:"21561",montantGarantie:new Double(0d),trans_totalht:new Double(15.12),trans_totaltva:new Double(15.12),trans_totalttc:new Double(15.12),bonCommande:null,partenaire:testInstances9[0],utilisateur:testInstancesu[0],deviss:testInstancesd,ligneProduits:testInstanceslp)]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(BonPret, testInstancesbp) 
        def testService=new BonPretService()
        def testInstancespa = [new PaternCompteur(libelle : "Pattern bon commande", prefixe : "BC{annee}{mois}{jour}", suffixe : "",pas:1,remplissage:12,numeroSuivant:2290,type:"BON_COMMANDE")]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(PaternCompteur, testInstancespa)
        def testId="bp001"
        def paternCompteurControl = mockFor(PaternCompteurService)
paternCompteurControl.demand.getProchainNumBpEtIncrementer(1..1) {it -> return  testId}
testService.paternCompteurService = paternCompteurControl.createMock()
        
        
     
def retval1 = testService.save(testInstancesbp[0])   
       assertTrue  retval1 instanceof BonPret
  System.out.println("la fin du tests"+retval1)       
    ...
    void testGetSequenceSuivante() {
     ...
     def testInstances1c = [new CategoriePartenaire(libelle : "propr", categorieParente : null)]
        ...
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        //mockDomain(PaternCompteur, testInstancespa)
        def retval1 = testService.getSequenceSuivante(testInstancesbp[0])   
        System.out.println("la fin du tests"+retval1)  
       //assertTrue  retval1 instanceof BonPret
