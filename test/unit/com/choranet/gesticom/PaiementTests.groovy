...
class PaiementTests extends GrailsUnitTestCase {
    ...
    void testPaiementContraintes() {
     ...
     def testInstances9p = [new Partenaire(code : "005", raisonSociale : "celibataire", idFiscal : null,patente:"15165165",registreCommerce:"commerce",cnss:"fk13252152",adresse:"oulfa",codePostal:"20200",telephone:"0673327392",fax:"0632972654",email:"contact@choranet.com",siteWeb:"www.choranet.com",adresseFacturation:"hay el hana",adresseLivraison:"ain sebaii",estClient:new Boolean(true),estFournisseur:new Boolean(false),estParticulier:new Boolean(false),estBlacklist:new Boolean(false),plafond:new Double(12.12),modeRelancePaiement:testInstances2m[0],modeReglementDefaut:testInstances3m[0],echeance:testInstances4e[0],ville:testInstances6v[0],formeJuridique:testInstances7f[0],modeLivraison:testInstances8m[0],categoriePartenaires:testInstances1c,solde:new Double(15.01),compteVerrouile:new Boolean(true),zone:testInstancesz[0],commercial:testInstancesee[0])]        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        ...
        def testInstancespa = [new Paiement(numTransaction : "05554", date :sqlDate1 , montantPaye : new Double(100),dateEncaissement:sqlDate1,credit:new Boolean(true),statut:"ouvert",modeReglement:testInstances[0],compteBancairePartenaire:null,compteBancaire:null,bonCommande:testInstancesbc[0],partenaire:testInstances9p[0])]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(Paiement, testInstancespa)
        assertEquals 1, Paiement.count()
        assertTrue testInstancespa[0] instanceof Paiement       
        def Paiement2 = new Paiement()
        assertFalse Paiement2.validate()
        assertEquals "nullable", Paiement2.errors["date"]
        assertEquals "nullable", Paiement2.errors["credit"]
        assertEquals "nullable", Paiement2.errors["modeReglement"]
        assertEquals "nullable", Paiement2.errors["bonCommande"]
        assertEquals "nullable", Paiement2.errors["partenaire"]
        def Paiement4 = new Paiement(numTransaction : "05554", date :sqlDate1 , montantPaye : new Double(100),dateEncaissement:sqlDate1,credit:new Boolean(true),statut:"ouvert",modeReglement:testInstances[0],compteBancairePartenaire:null,compteBancaire:null,bonCommande:testInstances[0],partenaire:testInstances9p[0])
        assertTrue Paiement4.validate()
        Paiement4.save(flush:true)
        assertEquals 2, Paiement.count()
        assertTrue Paiement.get(Paiement4.id) instanceof Paiement
        assertNotNull Paiement4.toString()
        assertTrue Paiement4.montantPaye >= 0d
    }
}
