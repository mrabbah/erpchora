...
import java.text.*
class LigneInventaireTests extends GrailsUnitTestCase {
    ...
    void testLigneInventaireContraintes() {
       ...
       def testInstances9 = [new Partenaire(code : "005", raisonSociale : "celibataire", idFiscal : null,patente:"15165165",registreCommerce:"commerce",cnss:"fk13252152",adresse:"oulfa",codePostal:"20200",telephone:"0673327392",fax:"0632972654",email:"contact@choranet.com",siteWeb:"www.choranet.com",adresseFacturation:"hay el hana",adresseLivraison:"ain sebaii",estClient:new Boolean(true),estFournisseur:new Boolean(false),estParticulier:new Boolean(false),estBlacklist:new Boolean(false),plafond:new Double(12.12),modeRelancePaiement:testInstances2[0],modeReglementDefaut:testInstances3[0],echeance:testInstances4[0],ville:testInstances6[0],formeJuridique:testInstances7[0],modeLivraison:testInstances8[0],categoriePartenaires:testInstances1,solde:new Double(15.01),compteVerrouile:new Boolean(true),zone:testInstancesz[0],commercial:testInstancesee[0])]        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        ...
        assertTrue testInstances10[0].validate()
        testInstances10[0].save(flush:true)
        def testInstances0 = [new Inventaire(dateDebut : sqlDate11, dateFin : sqlDate22, details : "trimestre",etat:"EN COURS",entrepot:testInstances10[0],ligneInventaires:null)]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(Inventaire, testInstances0)
        assertTrue testInstances0[0].validate()
        testInstances0[0].save(flush:true)
        
        
        def testInstances = [new LigneInventaire(quantiteInv : new Double(15.12), qantiteActuele : new Double(15.12), date_preremption : sqlDate11,trans_style:"commun",produit:testInstances20[0],inventaire:testInstances0[0])]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(LigneInventaire, testInstances)
        assertEquals 1, LigneInventaire.count()
        assertTrue testInstances[0] instanceof LigneInventaire        
        def LigneInventaire2 = new LigneInventaire()
        assertFalse LigneInventaire2.validate()
        assertEquals "nullable", LigneInventaire2.errors["quantiteInv"]
        assertEquals "nullable", LigneInventaire2.errors["qantiteActuele"]
        assertEquals "nullable", LigneInventaire2.errors["produit"]
        def LigneInventaire4 = new LigneInventaire(quantiteInv : new Double(15.12), qantiteActuele : new Double(15.12), date_preremption : sqlDate11,trans_style:"commun",produit:testInstances10[0],inventaire:testInstances0[0])
        assertTrue LigneInventaire4.validate()
        LigneInventaire4.save(flush:true)
        assertEquals 2, LigneInventaire.count()
        assertTrue LigneInventaire.get(LigneInventaire4.id) instanceof LigneInventaire
        assertNotNull LigneInventaire4.toString()
    }
}
