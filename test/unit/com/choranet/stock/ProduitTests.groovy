...
import java.text.*
class ProduitTests extends GrailsUnitTestCase {
    ...
    void testProduitContraintes() {
       ...
       def testInstances9 = [new Partenaire(code : "005", raisonSociale : "celibataire", idFiscal : null,patente:"15165165",registreCommerce:"commerce",cnss:"fk13252152",adresse:"oulfa",codePostal:"20200",telephone:"0673327392",fax:"0632972654",email:"contact@choranet.com",siteWeb:"www.choranet.com",adresseFacturation:"hay el hana",adresseLivraison:"ain sebaii",estClient:new Boolean(true),estFournisseur:new Boolean(false),estParticulier:new Boolean(false),estBlacklist:new Boolean(false),plafond:new Double(12.12),modeRelancePaiement:testInstances2[0],modeReglementDefaut:testInstances3[0],echeance:testInstances4[0],ville:testInstances6[0],formeJuridique:testInstances7[0],modeLivraison:testInstances8[0],categoriePartenaires:testInstances1,solde:new Double(15.01),compteVerrouile:new Boolean(true),zone:testInstancesz[0],commercial:testInstancesee[0])]        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        ...
        assertTrue testInstances20[0] instanceof Produit        
        def Produit2 = new Produit()
        assertFalse Produit2.validate()
        assertEquals "nullable", Produit2.errors["code"]
        assertEquals "nullable", Produit2.errors["designation"]
        assertEquals "nullable", Produit2.errors["prixRevient"]
        assertEquals "nullable", Produit2.errors["prixVenteStandard"]
        assertEquals "nullable", Produit2.errors["prixAchat"]
        assertEquals "nullable", Produit2.errors["empReception"]
        assertEquals "nullable", Produit2.errors["categorieProduit"]
        assertEquals "nullable", Produit2.errors["uniteMesure"]
        assertEquals "nullable", Produit2.errors["regimeTVA"]
        def Produit3 = new Produit(code : "0151513251", codebarre : "xxl01222", codecommun : "nbjbhb2555",profil:"alimentaire",designation:"marjane",prixRevient:new Double(14.12),prixVenteStandard:new Double(14.12),poids:new Double(14.12),prixAchat:new Double(14.12),prixMoyenPendere:new Double(14.12),perissable:new Boolean(false),temperature_conservation:new Double(52.12),imageProd:byte1,trans_imageProd:i,empReception:testInstances10[0],nature:testInstances11[0],categorieProduit:testInstances12[0],uniteMesure:testInstances13[0],regimeTVA:testInstances14[0],produitParent:null,fournisseurPref:testInstances9[0])
        assertFalse Produit3.validate()
        assertEquals "unique", Produit3.errors["code"]
        assertEquals "unique", Produit3.errors["codebarre"]
        def Produit4 = new Produit(code : "01515132512", codebarre : "xxl012223", codecommun : "nbjbhb2555",profil:"alimentaire",designation:"marjane",prixRevient:new Double(14.12),prixVenteStandard:new Double(14.12),poids:new Double(14.12),prixAchat:new Double(14.12),prixMoyenPendere:new Double(14.12),perissable:new Boolean(false),temperature_conservation:new Double(52.12),imageProd:byte1,trans_imageProd:i,empReception:testInstances10[0],nature:testInstances11[0],categorieProduit:testInstances12[0],uniteMesure:testInstances13[0],regimeTVA:testInstances14[0],produitParent:null,fournisseurPref:testInstances9[0])
        assertTrue Produit4.validate()
        Produit4.save(flush:true)
        assertEquals 2, Produit.count()
        assertTrue Produit.get(Produit4.id) instanceof Produit
        assertEquals "marjane" + " : " + "01515132512", Produit4.toString()
    }
}
