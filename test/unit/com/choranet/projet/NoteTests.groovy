...
//        testInstances[0].save(flush:true)
        ...
        def testInstancesn = [new Note(date : sqlDate1, objet : "reunion", corps : "compte rendu",projet:testInstancespj[0])]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(Note, testInstancesn)
        assertEquals 1, Note.count()
        assertTrue testInstancesn[0] instanceof Note      
        def Note2 = new Note()
        assertFalse Note2.validate()
        assertEquals "nullable", Note2.errors["date"]
        assertEquals "nullable", Note2.errors["objet"]
        assertEquals "nullable", Note2.errors["projet"]
        def Note4 = new Note(date : sqlDate1, objet : "reunion", corps : "compte rendu",projet:testInstancespj[0])
        assertTrue Note4.validate()
        Note4.save(flush:true)
        assertEquals 2, Note.count()
        System.out.println("nbjbjb"+Note4.toString())
        assertTrue Note.get(Note4.id) instanceof Note
        assertEquals "reunion", Note4.toString()
    }
}
