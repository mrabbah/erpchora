...
import java.text.*
class LocationOutilTests extends GrailsUnitTestCase {
   ...
   void testLocationOutilContraintes() {
        ...
        def testInstanceso = [new OutilLouable(designation : "fabrication", prixHeureLocation : new Double(500.05), loueur : testInstancesl[0])]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
...
//        testInstances[0].save(flush:true)
        ...
        def testInstanceslo = [new LocationOutil(date : sqlDate1, dureeLocation : new Integer(20), prixHeureLocation : new Double(12.15),prixForfait:new Double(12.15),total:new Double(15.02),loueur:testInstancesl[0],projet:testInstancespj[0],outilLouable:testInstanceso[0])]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(LocationOutil, testInstanceslo)
        assertEquals 1, LocationOutil.count()
        assertTrue testInstanceslo[0] instanceof LocationOutil        
        def LocationOutil2 = new LocationOutil()
        assertFalse LocationOutil2.validate()
        assertEquals "nullable", LocationOutil2.errors["date"]
        assertEquals "nullable", LocationOutil2.errors["dureeLocation"]
        assertEquals "nullable", LocationOutil2.errors["loueur"]
        assertEquals "nullable", LocationOutil2.errors["projet"]
        assertEquals "nullable", LocationOutil2.errors["outilLouable"]
        def LocationOutil4 = new LocationOutil(date : sqlDate1, dureeLocation : new Integer(20), prixHeureLocation : new Double(12.15),prixForfait:new Double(12.15),total:new Double(15.02),loueur:testInstancesl[0],projet:testInstancespj[0],outilLouable:testInstanceso[0])
        assertTrue LocationOutil4.validate()
        LocationOutil4.save(flush:true)
        assertEquals 2, LocationOutil.count()
        assertTrue LocationOutil.get(LocationOutil4.id) instanceof LocationOutil
        assertNotNull LocationOutil4.toString()
        assertTrue LocationOutil4.dureeLocation >= 0d
        assertTrue LocationOutil4.prixHeureLocation >= 0d
         assertTrue LocationOutil4.prixForfait >= 0d
    }
}
