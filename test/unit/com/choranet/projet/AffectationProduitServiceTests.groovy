...
class AffectationProduitServiceTests extends GrailsUnitTestCase {
    ...
    protected void tearDown() {
     ...
     void testSave() {
        ...
        mockDomain(AffectationProduit, testInstancesap) 
         ...
         def testInstancesss = [new Securitestock(stockMin : new Double(14.15), stockMax : new Double(14.15), stockReel : new Double(14.15),stockEntre:new Double(14.15),stockSortie:new Double(14.15),date_preremption:sqlDate1,entrepot:testInstances10[0],produit:testInstances20[0])]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(Securitestock, testInstancesss)
        
        def testInstances66 = [new MouvementStock(date : sqlDate1, quantite : new Double(14.12), typeMouvement : "ACHAT",date_preremption:sqlDate1,prixDeduitMvm:new Double(14.12),empSource:testInstances10[0],empDestination:testInstances10[0],produit:testInstances20[0],utilisateur:testInstancesu[0])]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(MouvementStock, testInstances66)
        assertTrue testInstances66[0].validate()
        testInstances66[0].save(flush:true)
        
        def testInstancesa = [new AlertsSecuriteStock(dateAlerte :sqlDate1 , dateFinAlerte : sqlDate1, tjsValide : new Boolean(true),stockMin:new Double(12.15),stockMax:new Double(12.15),stockReel:new Double(12.15),details:"depassement du delai",raisonDeclanchement:testInstances66[0],raisonRelachement:testInstances66[0])]
        //mockForConstraintsTests(DroitUtilisateur, testInstances)
        mockDomain(AlertsSecuriteStock, testInstancesa)
        
        def testService =new AffectationProduitService()
        def projetServiceControl = mockFor(ProjetService)
        def mouvementStockServiceControl = mockFor(MouvementStockService)
         def SecuritestockServiceControl = mockFor(SecuritestockService)
          def alertsSecuriteStockServiceControl = mockFor(AlertsSecuriteStockService)
        
        def most=new MouvementStockService()
        def sest=new SecuritestockService()
        //def alertsSecuriteStockServiceControl = mockFor(AlertsSecuriteStockService)
        projetServiceControl.demand.mettreAjourChargesReellesProjet(testInstancespj[0]) {return testInstancespj[0] }
        mouvementStockServiceControl.demand.ajouterMouvementAffectationProduit(testInstancesap[0]) {return testInstances66[0] }
        SecuritestockServiceControl.demand.mettreAjourSecuriteStockApresMouvement(testInstances66[0]) {testInstances66[0] }
        alertsSecuriteStockServiceControl.demand.mettreAjourAlertSecuriteStockApresMouvement(testInstances66[0],null,testInstancesss[0]) {testInstances66[0] }
        //alertsSecuriteStockService = alertsSecuriteStockServiceControl.createMock()
        most.securitestockService=SecuritestockServiceControl.createMock()
        testService.projetService = projetServiceControl.createMock()
        testService.mouvementStockService = mouvementStockServiceControl.createMock()
        sest.alertsSecuriteStockService = alertsSecuriteStockServiceControl.createMock()
        
        def myCriteria = [
    list : {Closure  cls -> return testInstances}
]
Securitestock.metaClass.static.createCriteria = { myCriteria }
        def f1=testService.save(testInstancesap[0])
        System.out.println("hjhggbgg alalalallalalalalall"+f1.toString())
    }
}
